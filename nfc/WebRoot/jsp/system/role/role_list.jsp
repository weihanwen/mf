				...
				<c:forEach items="${roleList_z}" var="var" varStatus="vs">
					...
					<c:forEach items="${gysqxlist}" var="varG" varStatus="vsG">
 					...
 					<tr>
						...
						<c:if test="${QX.edit == 1 }">
							<td class='center' style="width:100px;"><a onclick="roleButton('${var.ROLE_ID }','add_qx');" class="btn btn-warning btn-mini" title="分配新增权限">新增</a></td>
							<td class='center' style="width:100px;"><a onclick="roleButton('${var.ROLE_ID }','del_qx');" class="btn btn-warning btn-mini" title="分配删除权限">删除</a></td>
							<td class='center' style="width:100px;"><a onclick="roleButton('${var.ROLE_ID }','edit_qx');" class="btn btn-warning btn-mini" title="分配修改权限">修改</a></td>
							<td class='center' style="width:100px;"><a onclick="roleButton('${var.ROLE_ID }','cha_qx');" class="btn btn-warning btn-mini" title="分配查看权限">查看</a></td>
						</c:if>
						<td style="width:200px;">
			 				...
			 				<c:if test="${QX.edit == 1 }">
								...
								<a class='btn btn-mini btn-info' title="编辑" onclick="editRole('${var.ROLE_ID }');">编辑></a>
							</c:if>
						<c:choose> 
							<c:when test="${var.ROLE_ID == '2' or var.ROLE_ID == '1'}"> </c:when>
							<c:otherwise>
								 <c:if test="${QX.del == 1 }">
									 <a class='btn btn-mini btn-danger' title="删除" onclick="delRole('${var.ROLE_ID }','c','${var.ROLE_NAME }');">删除</a>
								 </c:if>
							</c:otherwise>
						</c:choose>
					</tr>
