								...
								<td style="width: 30px;" class="center">
									<div class='hidden-phone visible-desktop btn-group'>
										...
										<c:if test="${r"${QX.edit == 1 }"}">
											<a   title="编辑" onclick="edit('${r"${var."}${objectNameLower}_id${r"}"}');"  class="btn btn-mini btn-info"  ><i class="icon-edit"></i></a>
										</c:if>
										<c:if test="${r"${QX.del == 1 }"}">
											<a   title="删除" onclick="del('${r"${var."}${objectNameLower}_id${r"}"}');"  class="btn btn-mini btn-info"  ><i class="icon-trash"></i> </a>
										</c:if>
										<!--
